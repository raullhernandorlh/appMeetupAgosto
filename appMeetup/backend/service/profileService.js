...
async function updateOrganizerProfile(id, organizerProfile) {
    ...
    rows[0].forEach(row => {
        ...
        organizerProfile.organizer_province, organizerProfile.organizer_city, organizerProfile.description, row.id], function (error) {
            if (error) {
...
async function updateUserProfile(id, userProfile) {
    ...
    rows[0].forEach(row => {
        ...
        connection.query(sqlUpdateUsersProfile, [userProfile.first_name, userProfile.last_name, userProfile.user_image,
