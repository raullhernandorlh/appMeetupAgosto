...
async function userReservations(id) {
    ...
    const sqlUserReservations = "SELECT R.id, M.id, M.title, M.date, M.duration, M.time, M.location, M.meetup_principal_image, M.description, R.reservation_date,R.reservation_price, R.paid_out  FROM users U, reservations R, meetups M, organizerprofiles OP  WHERE R.id_user = U.id  AND R.id_meetup = M.id AND M.id = OP.id AND U.id = ? AND U.user_type = 'user'"
...
async function organizerReservations(id) {
    console.log(id);
    const sqlOrganizerReservations = "SELECT R.id, M.title, M.date, M.duration, M.time, M.location, M.meetup_principal_image, R.reservation_date,R.reservation_price, R.paid_out  FROM users U, reservations R, meetups M, organizerprofiles OP  WHERE R.id_user = U.id  AND R.id_meetup = M.id AND M.id = OP.id AND U.id = ? AND U.user_type = 'organizer'"
    try {
        const connection = await database.connection();
