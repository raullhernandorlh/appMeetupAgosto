...
const { userProfileDetail } = require('./profileService');
const { sendEmailRegister} = require('../utilities/sendEmail')
...
async function registerUser(register) {
    ...
    rows[0].forEach(row => {
        if (row.user_type === 'organizer') {
            ...
            connection.query(sqlInsertOrganizerUserProfile, [register.first_name, register.last_name, register.user_image, register.user_autonomous_community
                , register.user_province, register.user_city, register.phone, row.id], function (error) {
                    if (error) {
            ...
            connection.query(sqlInsertOrganizerOrganizerProfile, [register.organizer_name, register.organizer_autonomous_community,
            register.organizer_province, register.organizer_city, register.description, row.id], function (error) {
                if (error) {
        ...
        if (row.user_type === 'user') {
            ...
            connection.query(sqlInsertUserProfile, [register.first_name, register.last_name, register.user_image, register.user_autonomous_community
                , register.user_province, register.user_city, register.phone, row.id], function (error) {
                    if (error) {
                        const errorResponse = '{"error", "An error occurred while inserting the profile"}';
                        return errorResponse;
    ...
    sendEmailRegister(register.first_name,register.email)
        console.log("Email de confirmacion de registro mandado correctamente")
        console.log(register.first_name, register.email)
